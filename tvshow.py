...
def getID(APIKEY, title):
	try:
		URL = "https://imdb-api.com/en/API/SearchSeries/{}/{}".format(APIKEY, title)
		res = requests.get(URL).json()["results"][0]["id"]
...
def main():
	...
	listShows = [show.lower().replace(' ', '_').strip() for show in getList("template/showlist.md")]
